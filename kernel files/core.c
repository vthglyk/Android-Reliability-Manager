...
//VASILEIOS: changed H_table_dim and  H_table definitions and added U_table_dim and U_table
int H_table_dim = 0;
EXPORT_SYMBOL(H_table_dim);
long unsigned int *H_table;
EXPORT_SYMBOL(H_table);
int U_table_dim = 0;
EXPORT_SYMBOL(U_table_dim);
long unsigned int *U_table;
EXPORT_SYMBOL(U_table);
...
//DEFINE_PER_CPU(unsigned int, pid_gov); //cpufreq_reliability_TRUE.c, VASILEIOS: changed to current_sched_pid
DEFINE_PER_CPU(unsigned int, T_LI) = 40; 
...
//EXPORT_PER_CPU_SYMBOL(pid_gov);  //VASILEIOS: changed to current_sched_pid
EXPORT_PER_CPU_SYMBOL(T_LI);
...
inline void sample_values(void){	
	...
	__get_cpu_var(monitor_stats_data)[__get_cpu_var(monitor_stats_index)].HL_flag 		= __get_cpu_var(HL_flag);
