...
extern long unsigned int search_in_U_table(int pid);
...
#ifdef RELIABILITY_SCHED_ON
	
	if(U_table_dim > 0 && search_in_U_table(p->pid)) {
		*target_cpu = hmp_select_faster_cpu(p, cpu);	//VASILEIOS: We select a faster cpu
		return 1;
	}
	else {
		return 0;
...
static unsigned int hmp_idle_pull(int this_cpu)
{
	...
	if (!target->active_balance && task_rq(p) == target && 
		(U_table_dim == 0 || search_in_U_table(p->pid) != 0 || p->parent->pid <= 2)) {	//VASILEIOS: We allow migration only for high priority tasks and kernel tasks
		get_task_struct(p);
		target->active_balance = 1;
