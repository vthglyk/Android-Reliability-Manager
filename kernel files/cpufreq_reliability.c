...
static void do_dbs_timer(struct work_struct *work)
{
	...
	int idle_task[CPUS_PER_CLUSTER];
	int HL_in_cluster = 0;
	int all_idle = 1;
	int safe_mode = 0;
	...
	if (!dbs_tuners_ins.powersave_bias ||
		...
		for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
			if(idle_task[cpu_id] == 0)
				all_idle = 0;
		}
		
		if(all_idle == 1 || safe_mode == 1) {
			cluster_vf_index = Vf_APP_index_min[cpu / CPUS_PER_CLUSTER];
		}
		else  if(HL_in_cluster == 1){
			cluster_vf_index = 12;
		}
		else {
			cluster_vf_index = 12;
			//VASILEIOS: We assign the frequency corresponding to the lowest V_STC to preserve the reliability of all the cores
			for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
				int index = search_in_Vf_table( per_cpu(V_STC,cpu_id), cpu_id );
				if(index < cluster_vf_index) {
						cluster_vf_index = index;
				}	
			}
		}
		...
		//VASILEIOS: update T_LI
		if(cpu / CPUS_PER_CLUSTER == 0) {
			for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
				int temp = (temp_monitor[0] + temp_monitor[1] + temp_monitor[2] + temp_monitor[3] + temp_monitor[4]) / 5;
				per_cpu(T_LI,cpu_id) = ( per_cpu(T_LI,cpu_id) * per_cpu(t_LI,cpu_id)
									+ temp * per_cpu(delta_SI,cpu_id) ) 
									/ ( per_cpu(t_LI,cpu_id) + per_cpu(delta_SI,cpu_id) ) ;
			}		
		}
		else {
			for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
				...
				switch(cpu_id){
					...
					case 7:
						...
						break;   
					default : 
						printk (KERN_ALERT "VASILEIOS ALERT : You should not be here");
						break;
				}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
        ...
        for_each_cpu(j, policy->cpus) {
			...
			per_cpu(V_LTC,j) = Vf_table[cpu / CPUS_PER_CLUSTER][1][12]; //We start from the maximum voltage
			per_cpu(V_STC,j) = Vf_table[cpu / CPUS_PER_CLUSTER][1][12];
			per_cpu(V_LI,j) = Vf_table[cpu / CPUS_PER_CLUSTER][1][12];
