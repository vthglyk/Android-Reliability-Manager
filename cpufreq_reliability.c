...
static unsigned int search_in_Vf_table(unsigned int V_STC, int cpu);
long unsigned int search_in_H_table(int pid); 
long unsigned int search_in_U_table(int pid);
...
DECLARE_PER_CPU(unsigned int, T_LI);
DECLARE_PER_CPU(unsigned int, current_sched_pid); //pid_gov changed to current_sched_pid
...
unsigned long int Vf_table[NUM_CLUSTERS][2][13] = 
{
{
{900,900,900,900,925,963,1000,1038,1075,1113,1150,1200,1250},
{200000,300000,400000,500000,600000,700000,800000,900000,1000000,1100000,1200000,1300000,1400000}
},
{
{900,925,950,975,1000,1025,1038,1063,1100,1138,1175,1225,1300},
{800000,900000,1000000,1100000,1200000,1300000,1400000,1500000,1600000,1700000,1800000,1900000,2000000}
}
};

unsigned long int V_MAX[NUM_CLUSTERS] = {1250,1300};
unsigned long int V_MIN[NUM_CLUSTERS] = {1075,1000};
...
unsigned long int Vf_APP_index_min[NUM_CLUSTERS] = {8, 4};
//BILL
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	if (!dbs_tuners_ins.powersave_bias ||
		...
		if(per_cpu(V_LI,cpu) < V_MIN[cpu / CPUS_PER_CLUSTER])
			per_cpu(V_LI,cpu) = V_MIN[cpu / CPUS_PER_CLUSTER];
		if(per_cpu(V_LTC,cpu) < V_MIN[cpu / CPUS_PER_CLUSTER])
			per_cpu(V_LTC,cpu) = V_MIN[cpu / CPUS_PER_CLUSTER];
		...
		if ( per_cpu(V_STC,cpu) < V_MIN[cpu / CPUS_PER_CLUSTER] )
			per_cpu(V_STC,cpu) = V_MIN[cpu / CPUS_PER_CLUSTER] ; 
		if ( per_cpu(V_STC,cpu) > V_MAX[cpu / CPUS_PER_CLUSTER])
			per_cpu(V_STC,cpu) = V_MAX[cpu / CPUS_PER_CLUSTER] ;
		...
		//printk (KERN_ALERT "BILL ALERT : CPU%d activate_safe_mode=%u with pid = %u\n", cpu_id,per_cpu(activate_safe_mode,cpu_id), per_cpu(current_sched_pid,cpu_id));
		for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
			if (per_cpu(activate_safe_mode,cpu_id)!=1){
				if (per_cpu(current_sched_pid,cpu_id) == 0) {  //IDLE task
						per_cpu(Vf_APP_index,cpu_id) = Vf_APP_index_min[cpu_id / CPUS_PER_CLUSTER];
						per_cpu(HL_flag,cpu_id) = 0;
				}
				else{

					//per_cpu(HL_flag,cpu_id) = search_in_H_table(per_cpu(current_sched_pid,cpu_id));
					per_cpu(HL_flag,cpu_id) = search_in_U_table(per_cpu(current_sched_pid,cpu_id));
					
					//decide based on the table of voltage and frequencies and the HL flag
					if ( per_cpu(HL_flag,cpu_id) == 1 ) {
						per_cpu(Vf_APP_index,cpu_id) = 12; //the last one is 12
						printk (KERN_ALERT "BILL ALERT : CPU%d executes high priority job with pid = %u\n", cpu_id, per_cpu(current_sched_pid,cpu_id));
					}
					else if ( per_cpu(HL_flag,cpu_id) == 0 ) {	
						per_cpu(Vf_APP_index,cpu_id) = search_in_Vf_table( per_cpu(V_STC,cpu_id), cpu_id );
						if(per_cpu(Vf_APP_index,cpu_id) < Vf_APP_index_min[cpu_id / CPUS_PER_CLUSTER])
							per_cpu(Vf_APP_index,cpu_id) = Vf_APP_index_min[cpu_id / CPUS_PER_CLUSTER];
						//printk (KERN_ALERT "BILL ALERT : CPU%d has Vf_APP_index = %d", cpu_id, per_cpu(Vf_APP_index,cpu_id));
					}
				}
			}
			else{
			...
			if( per_cpu(Vf_APP_index,cpu_id) > cluster_vf_index )
				cluster_vf_index = per_cpu(Vf_APP_index,cpu_id);
				
		}
		
		for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
			per_cpu(Vf_APP_index,cpu_id) = cluster_vf_index;
			per_cpu(V_APP,cpu_id) = Vf_table[cpu_id / CPUS_PER_CLUSTER][0][ per_cpu(Vf_APP_index,cpu_id) ];
			per_cpu(f_APP,cpu_id) = Vf_table[cpu_id / CPUS_PER_CLUSTER][1][ per_cpu(Vf_APP_index,cpu_id) ];
			
			if (per_cpu(V_APP,cpu_id) > V_MAX[cpu_id/CPUS_PER_CLUSTER])
				per_cpu(V_APP,cpu_id) = V_MAX[cpu_id/CPUS_PER_CLUSTER] ; 
			if (per_cpu(V_APP,cpu_id) < V_MIN[cpu_id/CPUS_PER_CLUSTER])
				per_cpu(V_APP,cpu_id) = V_MIN[cpu_id/CPUS_PER_CLUSTER] ;
			
			if (per_cpu(f_APP,cpu_id) > f_MAX[cpu_id/CPUS_PER_CLUSTER])
				per_cpu(f_APP,cpu_id) = f_MAX[cpu_id/CPUS_PER_CLUSTER] ; 
			if (per_cpu(f_APP,cpu_id) < f_MIN[cpu_id/CPUS_PER_CLUSTER])
				per_cpu(f_APP,cpu_id) = f_MIN[cpu_id/CPUS_PER_CLUSTER] ;		
		}
		// fake performance governor
		//per_cpu(V_APP,cpu) = Vf_table[0][ 12 ];
		//per_cpu(f_APP,cpu) = Vf_table[1][ 12 ];
		...
		if  ( ( per_cpu(t_LI,cpu) + per_cpu(delta_SI,cpu)  ) == 0  )
			printk (KERN_ALERT "PIETRO ALERT : ( CPU%d: per_cpu(t_LI) + per_cpu(delta_SI) == 0 , %lu , %lu" 
								,cpu, per_cpu(t_LI,cpu), per_cpu(delta_SI,cpu));
		
		for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
			if ( per_cpu(t_LI,cpu_id) < per_cpu(delta_LI,cpu_id) )
				per_cpu(V_LI,cpu_id) = ( per_cpu(V_LI,cpu_id) * per_cpu(t_LI,cpu_id) 
									+ per_cpu(V_APP,cpu_id) * per_cpu(delta_SI,cpu_id) ) 
									/ ( per_cpu(t_LI,cpu_id) + per_cpu(delta_SI,cpu_id) ) ;
								
			
		}
		
		//BILL: update T_LI
		if(cpu / CPUS_PER_CLUSTER == 0) {
			for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
				per_cpu(T_LI,cpu_id) = (temp_monitor[0] + temp_monitor[1] + temp_monitor[2] + temp_monitor[3] + temp_monitor[4]) / 5;
			}		
		}
		else {
			for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
				switch(cpu_id){
					case 4:
						per_cpu(T_LI,cpu_id) = temp_monitor[0];
						break; 
					case 5:
						per_cpu(T_LI,cpu_id) = temp_monitor[3];
						break; 
					case 6:
						per_cpu(T_LI,cpu_id) = temp_monitor[2];
						break; 
					case 7:
						per_cpu(T_LI,cpu_id) = temp_monitor[1];
						break;   
					default : 
						printk (KERN_ALERT "BILL ALERT : You should not be here");
						break;
				}
				
			}
		}
		//printk(KERN_ALERT "BILL: T_LI = %lu, %u, %u, %u, %u, %u", T_LI, temp_monitor[0], temp_monitor[1], temp_monitor[2], temp_monitor[3], temp_monitor[4]);
		
		
		//BILL
		//set the new frequency
		/* printk (KERN_ALERT "BILL ALERT : CPU%d New f = %lu and V = %lu\n" 
									,cpu, per_cpu(f_APP,cpu), per_cpu(V_APP,cpu)); */ 
		...
		if( per_cpu(t_LI,cpu)%100 == 0){
			printk (KERN_ALERT "PIETRO DEBUG EXEC TIME CHANGE FREQUENCY : c_duration_change_freq = %lu in %u and t_LI = %lu\n" , c_duration_change_freq, cpu, per_cpu(t_LI,cpu));
			printk (KERN_ALERT "BILL ALERT : CPU%d New V_LI = %lu, V_STC = %lu, t_LI = %lu, V_APP = %lu, f_APP = %lu, curr_pid = %lu, HL = %u, cluster_vf_index = %u\n" 
									,cpu, per_cpu(V_LI,cpu),per_cpu(V_STC,cpu), per_cpu(t_LI,cpu), per_cpu(V_APP,cpu), 
									per_cpu(f_APP,cpu), per_cpu(current_sched_pid,cpu), per_cpu(HL_flag,cpu), cluster_vf_index); 
		}
	...
	for(cpu_id = cpu; cpu_id < cpu + CPUS_PER_CLUSTER; cpu_id++) {
		if ( per_cpu(jiffies_prec , cpu_id) == 0){
			per_cpu(jiffies_prec , cpu_id) = jiffies - 1 ;
		}
		
		per_cpu(t_LI,cpu_id) = per_cpu(t_LI,cpu_id) + (  jiffies - per_cpu(jiffies_prec , cpu_id) );
		per_cpu(jiffies_prec , cpu_id) = jiffies;
	}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		/* for(b1=0; b1<NUM_CLUSTERS; b1++)			//BILL: print Vf_table
			for(b2=0; b2<2; b2++)
				for(b3=0; b3<13; b3++)
					printk(KERN_ALERT "BILL ALERT : Vf_table[%d][%d][%d] = %lu\n" , b1, b2, b3, Vf_table[b1][b2][b3]); */
		#ifdef CONFIG_RELIABILITY
        ...
        for_each_cpu(j, policy->cpus) {
			...
			per_cpu(V_LTC,j) = V_MIN[cpu / CPUS_PER_CLUSTER];//000;    //
			per_cpu(V_STC,j) = V_MIN[cpu / CPUS_PER_CLUSTER];//000;
			per_cpu(V_LI,j) = V_MIN[cpu / CPUS_PER_CLUSTER];//000;
...
static unsigned int search_in_Vf_table(unsigned int V_STC, int cpu){
	...
	unsigned int i, Vf_APP_index = 0;


	for (i=0; i<13; i++){
		if (Vf_table[cpu / CPUS_PER_CLUSTER][0][i] > V_STC)
			diff = Vf_table[cpu / CPUS_PER_CLUSTER][0][i] - V_STC;
		else diff = V_STC - Vf_table[cpu / CPUS_PER_CLUSTER][0][i] ;
